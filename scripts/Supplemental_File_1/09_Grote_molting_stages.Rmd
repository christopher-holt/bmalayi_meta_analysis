---
title: "Reanalysis of Grote, A. Motifs associated with molting stages, PRJNA557263"
...
generate_rarefaction_curve <- function(counts.mat, groups.df, file_name, group_classification, colour){
  ...
  write_tsv(rarefy.df, paste0(FILE_DIR, "/rarefy.df.tsv"), col_names = T)
                                                                                                                                                                     
  write_tsv(rarefy.points.df, paste0(FILE_DIR, "/rarefy.points.df.tsv"), col_names = T)
...
## Stage Specific Data
PROJECT="grote_PRJNA344486"
STAGE_SPECIFIC_COUNTS_URL <- paste0('https://raw.githubusercontent.com/christopher-holt/bmalayi_meta_analysis/main/input_files/', PROJECT, '/combined.final.counts')
STAGE_SPECIFIC_GROUPS_URL <- paste0('https://raw.githubusercontent.com/christopher-holt/bmalayi_meta_analysis/main/input_files/', PROJECT, '/groups.txt')
```
...
## Need the L4 data from this study
stage_specific_groups <- utils::read.table(STAGE_SPECIFIC_GROUPS_URL, sep = ',')
stage_specific_counts <- utils::read.table(STAGE_SPECIFIC_COUNTS_URL)
...
groups <- groups %>% mutate(SampleColour = ifelse(groups$Sample == "Bm.L3", "black",
                                                  ifelse(groups$Sample == "Bm.L3Day6", "grey", "blue")))
...
## Format stage_specific_groups
```{r}
stage_specific_groups <- as.data.frame(stage_specific_groups)
stage_specific_groups <- stage_specific_groups %>% select(-'V1') %>% distinct()
stage_specific_groups$Sample <- str_remove(string = stage_specific_groups$V2, pattern = '_[a-z]')
colnames(stage_specific_groups) <- c("Replicate", "SampleColour", "Project", "Sample")

stage_specific_groups <- stage_specific_groups %>% select("Replicate", "Project", "Sample", "SampleColour")
L4.groups <- stage_specific_groups %>% filter(Sample == "Bm.L4")
L4.counts <- stage_specific_counts %>% filter(grepl("Bm.L4", stage_specific_counts$V3))

groups <- rbind(groups, L4.groups)
groups <- factor_groups(groups)

counts.orig <- rbind(counts.orig, L4.counts)
...
pdf(paste0(FIGURE_DIR, "/rarefaction_curve.pdf"),
  ...
  labs(x="number of reads mapping to features annotated as mRNA", y="number genes detected", color = 'none', shape = 'Enrichment Method') +
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Grote et al 2020 Moulting')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Grote et al 2020 Moulting')
)
dev.off()
