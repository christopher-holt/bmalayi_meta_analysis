---
title: "Reanalysis of Choi, Y.J. Deep sequencing of lifecycle, PRJEB2709"
...
groups <- groups %>% mutate(Sample_1 = ifelse(grepl("microfilariae_a", groups$V2), "MM",
                                  ifelse(grepl("adult.female", groups$V2), "AF",
                                         ifelse(grepl("L4", groups$V2), "L4",
                                                ifelse(grepl("L3", groups$V2), "L3",
                                                       ifelse(grepl("immature.microfilaria", groups$V2), "IM",
                                                              ifelse(grepl("eggs.and.embryos", groups$V2), "EE",
                                                                     ifelse(grepl("adult.male", groups$V2), "AM", NA))))))))

colnames(groups) <- c("Replicate", "column_colour","Project","Sample", "Sample_1")

groups <- groups %>% arrange(desc(Sample))
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Choi et al 2011')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Choi et al 2011')
)
dev.off()
...
y <- edgeR::DGEList(counts = counts, group = groups$Sample_1)
y <- edgeR::calcNormFactors(y)
keep <- rowSums(cpm(y) >= cpm.cutoff) >= min(table(groups$Sample_1)) ## is the cpm > than cutoff in two or more replicates
...
design <- model.matrix(~groups$Sample_1)
...
pdf(paste0(FIGURE_DIR, "/PCA_of_samples.pdf"),
  ...
  print(
    ggplot()+
      geom_point(aes(x=pca$ind$coord[,1], y=pca$ind$coord[,2], color = groups$Sample_1), size = 10)+
      labs(col = "Samples", size = "Reads Mapped\nto Features", 
           x = paste0("PC1 (", round(pca$eig[1,2],1), "%)"), 
           y = paste0("PC2 (", round(pca$eig[2,2],1), "%)"))+
      guides(colour = guide_legend(ncol = 2))+
      scale_color_manual(values = levels(groups$column_colour))+
...
colcol <- as.character(groups$column_colour)
