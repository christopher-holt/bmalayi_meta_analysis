---
title: "Reanalysis of Grote, A., Defining stage specific expression, PRJNA344486"
...
colnames(groups) <- c("Replicate", "column_colour","Project", "Sample")
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Grote et al 2017')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Grote et al 2017')
)
dev.off()
...
## Plot log2TPM of genes passing edgeR cutoff but not necessarily differentially expressed
```{r}
