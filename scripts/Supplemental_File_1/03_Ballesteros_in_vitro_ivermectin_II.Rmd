---
title: "Reanalysis of Ballesteros, C., In vitro ivermectin study II, PRJNA303986"
...
plot_pca <- function(y, tpm, groups.df, OUTFILE){
  ...
  pdf(paste0(FIGURE_DIR, "/", OUTFILE, "_size_10.pdf"),
       width = 10,
       height = 10)
    print(
      ggplot()+
...
PROJECT="ballesteros_PRJNA303986"
cwd <- getwd()
...
counts.orig <- utils::read.table(COUNT_URL)
...
pdf(paste0(FIGURE_DIR, "/rarefaction_curve_linewidth_2_size_7.pdf"),
     width = 10,
     height = 10)
  print(
  ggplot()+
...
generate_rarefaction_curve(counts.mat = counts, groups.df = groups, file_name = "rarefaction_curve", group_classification = "IVM", colour = "IVM.Colour", shape_factor = "Isolation")
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Ballesteros et al IVM II 2016')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Ballesteros et al IVM II 2016')
)
dev.off()
...
## Plot PCA of all samples, only using genes that have a rowSum > 0, Figure 4D
```{r}
...
pca <- PCA(t(as.data.frame(t(scale(t(log2(tpm.keep+1)))))), graph = FALSE, ncp = ncol(counts) - 1)
pdf(paste0(FIGURE_DIR, "/PCA_all_samples_size_10.pdf"),
     width = 10,
     height = 10)
  print(
    ggplot()+
...
```{r}
find_de_genes <- function(groups.df, counts.mat){
  groups.df$grouping_factor <- groups.df$Sample
  groups.df <- factor_groups(groups.df)
  counts.mat <- counts.mat[,colnames(counts.mat) %in% groups.df$Replicate]
  degenes.list <- identify_de_genes(counts.mat = counts.mat, groups.df = groups.df)
}

find_de_genes(groups %>% filter((Isolation == "48h" | Isolation == "McGill") & IVM == "0nM"), counts)
...
```{r}
## Between 0nm and 300nm at 48h
groups.48h <- groups %>% filter(Isolation == "48h" & IVM != "1uM")
groups.48h$grouping_factor <- groups.48h$Sample
groups.48h <- factor_groups(groups.48h)
counts.48h <- counts[,colnames(counts) %in% groups.48h$Replicate]
counts.48h <- counts[,levels(groups.48h$Replicate)]
...
pdf(paste0(FIGURE_DIR, "/variancePartition_histogram_0nm_300nm_48h.pdf"),
     width = 10,
     height = 10)
print(
 ggplot(varPart.df.48h, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = '0nm vs 300nm at 48h')
)
dev.off()

pdf(paste0(FIGURE_DIR, "/variancePartition_0nm_300nm_48h.pdf"),
     width = 10,
     height = 10)
print(
  ggplot(varPart.df.48h, aes(x = attribute, y = Variance_Explained * 100)) + 
  geom_boxplot() + labs(y = 'Variance explained (%)', title = '0nm vs 300nm at 48h')
)
dev.off()
...
plot_pca(y.48h, tpm, groups.48h, "PCA_48h_0nm_300nm_comparison")
groups.48h <- plot_dendrogram(y = y.48h, tpm = tpm, groups = groups.48h, OUTFILE = "dendrogram_48h_0nm_300nm_comparison")



plot_de_genes(tpm = tpm, degenes = degenes.48h, groups.df = groups.48h, OUTFILE = "WGCNA_48h_0nm_300nm_all_log_transformed", column_colour = "Sample.Colour", y_dge = y.48h, wgcna_clustering = FALSE)
plot_de_genes(tpm = tpm, degenes = degenes.48h, groups.df = groups.48h, OUTFILE = "WGCNA_48h_0nm_300nm_all_zscore", column_colour = "Sample.Colour", y_dge = y.48h, wgcna_clustering = FALSE, z_score = TRUE)
```

```{r}
## Between 0nm and 1um at 48h
groups.48h <- groups %>% filter(Isolation == "48h" & IVM != '300nM')
groups.48h$grouping_factor <- groups.48h$Sample
groups.48h <- factor_groups(groups.48h)
counts.48h <- counts[,colnames(counts) %in% groups.48h$Replicate]
# Calculate Differentially Expressed Genes
degenes.48h <- identify_de_genes(counts.mat = counts.48h, groups.df = groups.48h)
et.degenes.48h <- identify_de_genes(counts.mat = counts.48h, groups.df = groups.48h, use_exactTest = TRUE)
...
pdf(paste0(FIGURE_DIR, "/variancePartition_histogram_0nm_1um_48h.pdf"),
     width = 10,
     height = 10)
print(
 ggplot(varPart.df.48h, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = '0nm vs 1um at 48h')
)
dev.off()

pdf(paste0(FIGURE_DIR, "/variancePartition_0nm_1um_48h.pdf"),
     width = 10,
     height = 10)
print(
  ggplot(varPart.df.48h, aes(x = attribute, y = Variance_Explained * 100)) + 
  geom_boxplot() + labs(y = 'Variance explained (%)', title = '0nm vs 1um at 48h')
)
dev.off()
...
plot_pca(y.48h, tpm, groups.48h, "PCA_48h_0nm_1um_comparison")
groups.48h <- plot_dendrogram(y = y.48h, tpm = tpm, groups = groups.48h, OUTFILE = "dendrogram_48h_0nm_1um_comparison")



plot_de_genes(tpm = tpm, degenes = degenes.48h, groups.df = groups.48h, OUTFILE = "WGCNA_48h_0nm_1um_all_log_transformed", column_colour = "Sample.Colour", y_dge = y.48h, wgcna_clustering = FALSE)
plot_de_genes(tpm = tpm, degenes = degenes.48h, groups.df = groups.48h, OUTFILE = "WGCNA_48h_0nm_1um_all_zscore", column_colour = "Sample.Colour", y_dge = y.48h, wgcna_clustering = FALSE, z_score = TRUE)
```
```{r}
## Between 0nm and 300nm at 5d
groups.5d <- groups %>% filter(Isolation == "5d" & IVM != '1uM')
groups.5d$grouping_factor <- groups.5d$Sample
groups.5d <- factor_groups(groups.5d)
counts.5d <- counts[,colnames(counts) %in% groups.5d$Replicate]
# Calculate Differentially Expressed Genes
degenes.5d <- identify_de_genes(counts.mat = counts.5d, groups.df = groups.5d)
et.degenes.5d <- identify_de_genes(counts.mat = counts.5d, groups.df = groups.5d, use_exactTest = TRUE)

y.5d <- identify_de_genes(counts.mat = counts.5d, groups.df = groups.5d, returnDGE = TRUE)


form <- ~ (1|IVM)
varPart.df.5d <- variance_function(counts.5d, groups.5d, form)

pdf(paste0(FIGURE_DIR, "/variancePartition_histogram_0nm_300nm_5d.pdf"),
     width = 10,
     height = 10)
print(
 ggplot(varPart.df.5d, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = '0nm vs 300nm at 5d')
)
dev.off()

pdf(paste0(FIGURE_DIR, "/variancePartition_0nm_300nm_5d.pdf"),
     width = 10,
     height = 10)
print(
  ggplot(varPart.df.5d, aes(x = attribute, y = Variance_Explained * 100)) + 
  geom_boxplot() + labs(y = 'Variance explained (%)', title = '0nm vs 300nm at 5d')
)
dev.off()

plot_pca(y.5d, tpm, groups.5d, "PCA_5d_0nm_300nm_comparison")
groups.5d <- plot_dendrogram(y = y.5d, tpm = tpm, groups = groups.5d, OUTFILE = "dendrogram_5d_0nm_300nm_comparison")


plot_de_genes(tpm = tpm, degenes = degenes.5d, groups.df = groups.5d, OUTFILE = "WGCNA_5d_0nm_300nm_all_log_transformed", column_colour = "Sample.Colour", y_dge = y.5d, wgcna_clustering = FALSE)
plot_de_genes(tpm = tpm, degenes = degenes.5d, groups.df = groups.5d, OUTFILE = "WGCNA_5d_0nm_300nm_all_zscore", column_colour = "Sample.Colour", y_dge = y.5d, wgcna_clustering = FALSE, z_score = TRUE)
```

```{r}
## Between 0nm and 1um at 5d
groups.5d <- groups %>% filter(Isolation == "5d" & IVM != '300nM')
groups.5d$grouping_factor <- groups.5d$Sample
groups.5d <- factor_groups(groups.5d)
counts.5d <- counts[,colnames(counts) %in% groups.5d$Replicate]
# Calculate Differentially Expressed Genes
...
pdf(paste0(FIGURE_DIR, "/variancePartition_histogram_0nm_1um_5d.pdf"),
     width = 10,
     height = 10)
print(
 ggplot(varPart.df.5d, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = '0nm vs 1um at 5d')
)
dev.off()

pdf(paste0(FIGURE_DIR, "/variancePartition_0nm_1um_5d.pdf"),
     width = 10,
     height = 10)
print(
  ggplot(varPart.df.5d, aes(x = attribute, y = Variance_Explained * 100)) + 
  geom_boxplot() + labs(y = 'Variance explained (%)', title = '0nm vs 1um at 5d')
)
dev.off()
...
plot_pca(y.5d, tpm, groups.5d, "PCA_5d_0nm_1um_comparison")
groups.5d <- plot_dendrogram(y = y.5d, tpm = tpm, groups = groups.5d, OUTFILE = "dendrogram_5d_1um_300nm_comparison")



plot_de_genes(tpm = tpm, degenes = degenes.5d, groups.df = groups.5d, OUTFILE = "WGCNA_5d_0nm_1um_all_log_transformed", column_colour = "Sample.Colour", y_dge = y.5d, wgcna_clustering = FALSE)
plot_de_genes(tpm = tpm, degenes = degenes.5d, groups.df = groups.5d, OUTFILE = "WGCNA_5d_0nm_1um_all_zscore", column_colour = "Sample.Colour", y_dge = y.5d, wgcna_clustering = FALSE, z_score = TRUE)
