---
title: "Reanalysis of Chung, M., Lifecycle transcriptome, PRJNA294263"
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Chung et al 2019 Bromodomain')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Chung et al 2019 Bromodomain')
)
dev.off()
...
pdf(paste0(FIGURE_DIR, "/PCA_tpm_de.pdf"))
  print(
    ggplot()+
      geom_point(aes(x=pca$ind$coord[,1], y=pca$ind$coord[,2], color = groups$Sample, shape = groups$Enrichment), size = 10)+
