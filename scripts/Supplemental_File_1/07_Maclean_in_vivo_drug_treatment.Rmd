---
title: "Reanalysis of Maclean, M.J., In vivo drug treatment, PRJNA388112"
...
functionaltermenrichment <- function(genes, geneinfo, functionalterms.list){
  ...
  for(i in 1:length(functionalterms.list)){
    for(j in 1:nrow(functionalterms.list[[i]])){
      ...
      freq.all <- functionalterms.list[[i]][j,2] ## How many times the IPR/GO term appears in the entire geneinfo file
...
generate_rarefaction_curve <- function(counts.mat, groups.df, file_name, group_classification, colour){

  rarefy.counts <- round(counts.mat,0)
  ...
  for(i in unique(rarefy.raw.df$Site)){
    ...
    rarefy.df <- as.data.frame(rbind(rarefy.df,
                                     cbind(as.numeric(steps),as.numeric(detected_genes),
                                           as.character(groups.df[groups.df$Replicate == i,]$Replicate),
                                           as.character(groups.df[groups.df$Replicate == i,][[group_classification]]),
                                           as.character(groups.df[groups.df$Replicate == i,][[colour]]))))
  ...
  col <- as.character(groups[[colour]])
  names(col) <- as.character(groups[[group_classification]])
  
  pdf(paste0(FIGURE_DIR, "/", file_name, "_linewidth_2_size_7.pdf"),
       width = 10,
       height = 10)
    print(
    ggplot()+
    ...
    labs(x="number of reads mapping to features annotated as mRNA", y="number genes detected", color = paste0(colour)) +
    theme_bw() + scale_y_continuous(expand = expansion(mult = c(0.2, .2))) + geom_hline(yintercept = nrow(counts)) + xlim(0,65000000)
...
find_de_genes <- function(groups.to.use, counts, grouping_factor, returnDGE = FALSE){
  ...
  if(returnDGE){
      y.to.return <- identify_de_genes(counts.mat = counts.to.use, groups.df = groups.to.use, returnDGE = TRUE)
      return(y.to.return)
...
colnames(groups) <- c("Replicate", "Treatment", "Replicate_Number", "LifeStage", "Exposure","Project", "Sample")

groups <- groups %>% mutate(Treatment_WormSex_Colour = ifelse(grepl("DMSO", groups$Treatment) & grepl("Female_worm", groups$LifeStage), "darkblue",
                                                              ifelse(grepl("DMSO", groups$Treatment) & grepl("^Male_worm", groups$LifeStage), "darkred",
                                                                     ifelse(grepl("DMSO", groups$Treatment) & grepl("Microfilaria", groups$LifeStage), "darkgreen",
                                                                            ifelse(!(grepl("DMSO", groups$Treatment)) & grepl("Female_worm", groups$LifeStage), "blue",
                                                                                   ifelse(!(grepl("DMSO", groups$Treatment)) & grepl("^Male_worm", groups$LifeStage), "red",
                                                                                          ifelse(!(grepl("DMSO", groups$Treatment)) & grepl("Microfilaria", groups$LifeStage), "green", "white")))))))


groups <- groups %>% mutate(Treatment_Colour = ifelse(grepl("Albendazole", groups$Treatment), "orange",
                                                     ifelse(grepl("DMSO", groups$Treatment), "green",
                                                            ifelse(grepl("Diethylcarbamazine", groups$Treatment), "purple",
                                                                   ifelse(grepl("Ivermectin", groups$Treatment), "pink", "NA")))))

groups <- groups %>% mutate(Worm_Colour = ifelse(grepl("Female_worm", groups$LifeStage), "blue",
                                                     ifelse(grepl("^Male_worm", groups$LifeStage), "red",
                                                            ifelse(grepl("Microfilaria", groups$LifeStage), "green", "NA"))))

groups <- groups %>% mutate(Exposure_Colour = ifelse(grepl("1_day", groups$Exposure), "grey",
                                                     ifelse(grepl("7_day", groups$Exposure), "black", "NA")))

groups <- groups %>% mutate(Replicate_Colour = ifelse(grepl("1", groups$Replicate_Number), "red",
                                                      ifelse(grepl("2", groups$Replicate_Number), "blue",
                                                             ifelse(grepl("3", groups$Replicate_Number), "green", "NA"))))

groups <- groups %>% mutate(Worm_Exposure_Colour = ifelse(groups$LifeStage == "Female_worm", "blue",
                                                                 ifelse(groups$LifeStage == "Male_worm", "red",
                                                                               ifelse(groups$LifeStage == "Microfilaria", "green", NA))))
...
groups.alb <- groups %>% filter(Treatment == "Albendazole")
groups.alb <- factor_groups(groups.alb)
counts.alb <- counts[,colnames(counts) %in% groups.alb$Replicate]

generate_rarefaction_curve(counts.mat = counts.alb, groups.df = groups.alb, file_name = "rarefaction_curve_alb", group_classification = "LifeStage", colour = "Worm_Colour")

groups.dec <- groups %>% filter(Treatment == "Diethylcarbamazine")
groups.dec <- factor_groups(groups.dec)
counts.dec <- counts[,colnames(counts) %in% groups.dec$Replicate]

generate_rarefaction_curve(counts.mat = counts.dec, groups.df = groups.dec, file_name = "rarefaction_curve_dec", group_classification = "LifeStage", colour = "Worm_Colour")

groups.ivm <- groups %>% filter(Treatment == "Ivermectin")
groups.ivm <- factor_groups(groups.ivm)
counts.ivm <- counts[,colnames(counts) %in% groups.ivm$Replicate]

generate_rarefaction_curve(counts.mat = counts.ivm, groups.df = groups.ivm, file_name = "rarefaction_curve_ivm", group_classification = "LifeStage", colour = "Worm_Colour")


groups.dmso <- groups %>% filter(Treatment == "DMSO")
groups.dmso <- factor_groups(groups.dmso)
counts.dmso <- counts[,colnames(counts) %in% groups.dmso$Replicate]

generate_rarefaction_curve(counts.mat = counts.dmso, groups.df = groups.dmso, file_name = "rarefaction_curve_dmso", group_classification = "LifeStage", colour = "Worm_Colour")
...
form <- ~ (1|Treatment) + (1|Exposure) + (1|LifeStage)
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Maclean et al 2019')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Maclean et al 2019')
)
dev.off()

median_variance(varPart.df.all, attr_list = c("Residuals", all.vars(form)))


```


## Identify DIfferentiall Expressed Genes
```{r}
...
## PCA
```{r}
pca <- PCA(t(as.data.frame(t(scale(t(log2(tpm.de+1)))))), graph = FALSE, ncp = ncol(tpm.de) - 1)
pdf(paste0(FIGURE_DIR, "/PCA_degenes.pdf"),
     width = 10,
     height = 10)
  print(
    ggplot()+
      geom_point(aes(x=pca$ind$coord[,1], y=pca$ind$coord[,2], color = groups$LifeStage, shape = groups$Treatment), size = 10)+
      ...
      scale_color_manual(values = levels(groups$Worm_Colour)) +
      theme_bw()
  )
...
colcol <- as.character(groups$Worm_Colour)
colcol1 <- as.character(groups$Treatment_Colour)
colcol2 <- as.character(groups$Exposure_Colour)
colcol3 <- cbind(colcol, colcol1, colcol2)
colnames(colcol3) <- c('LifeStage', 'TreatmentColour', 'ExposureColour')
...
heatmap.3(heatmap_matrix,
              ...
              RowSideColorsSize = 1,
              ColSideColorsSize = 3,
              ColSideColors=as.matrix(colcol3),
              lhei = c(2,8,1),
              breaks = breaks,
...
# Separate By Life Stage

## Males
```{r}
groups.male <- factor_groups(groups %>% filter(LifeStage == "Male_worm"))
counts.male <- counts[,colnames(counts) %in% groups.male$Replicate]
tpm.male <- tpm[,colnames(tpm) %in% groups.male$Replicate]

## Calculate Differential Expression
FDRcutoff <- 0.05
cpm.cutoff <- 5/min(colSums(counts.male)) * 1000000 ## list minimum counts per million
print(paste0(nrow(counts.male)," total genes"))
y <- edgeR::DGEList(counts = counts.male, group = groups.male$Sample)
y <- edgeR::calcNormFactors(y)
keep <- rowSums(cpm(y) >= cpm.cutoff) >= min(table(groups.male$Sample)) ## is the cpm > than cutoff in two or more replicates
keep.df <- as.data.frame(table(keep))
print(paste0(keep.df[keep.df[,1] == F,2]," genes excluded using edgeR filter"))
y <- y[keep, , keep.lib.sizes = F] ## Selects genes in y that are in keep
design <- model.matrix(~groups.male$Sample)
y <- estimateDisp(y , design)
## Next two steps use Genewise negative binomial generalised linear models with quasilikelihood tests
fit <- edgeR::glmQLFit(y, design) 
qlf <- edgeR::glmQLFTest(fit, coef = 2:ncol(fit)) 
...
tpm.male.keep <- tpm.male[rownames(tpm.male) %in% rownames(y$counts),]



## Generate Dendrogram
tpm_dendrogram <- pvclust::pvclust(as.data.frame(t(scale(t(log2(tpm.male.keep+1))))), method.dist="cor", method.hclust="average",
                                   nboot=1000, parallel=TRUE)

pdf(paste0(FIGURE_DIR, "/dendrogram_males_only.pdf"),
    width = 10,
    height = 10)
  plot(tpm_dendrogram)
invisible(dev.off())
...
sample_order <- factor(as.character(dendro_order$label), levels = as.character(dendro_order$label))
groups.male <- factor_groups(groups.male[order(match(groups.male$Replicate, as.character(sample_order))),])

rownames(groups.male) <- NULL

tpm.male.keep <- tpm.male.keep[,levels(groups.male$Replicate)]


## Plot genes that passed CPM filter
log2tpm.de <- log2(tpm.male.keep + 1)
zscore.log2tpm.de <- as.data.frame(t(scale(t(log2tpm.de))))
zscore.log2tpm.de <- zscore.log2tpm.de[,as.character(sample_order)]
...
colcol <- as.character(groups.male$Worm_Colour)
colcol1 <- as.character(groups.male$Treatment_Colour)
colcol2 <- as.character(groups.male$Exposure_Colour)
colcol3 <- cbind(colcol, colcol1, colcol2)
colnames(colcol3) <- c('LifeStage', 'TreatmentColour', 'ExposureColour')
...
pdf(paste0(FIGURE_DIR, "/heatmap_of_genes_in_males.pdf"),
    width = 20,
    height = 10)
heatmap.3(heatmap_matrix,
              col=hmcol,
...
## Female
```{r}
groups.female <- factor_groups(groups %>% filter(LifeStage == "Female_worm"))
counts.female <- counts[,colnames(counts) %in% groups.female$Replicate]
tpm.female <- tpm[,colnames(tpm) %in% groups.female$Replicate]

## Calculate Differential Expression
FDRcutoff <- 0.05
cpm.cutoff <- 5/min(colSums(counts.female)) * 1000000 ## list minimum counts per million
print(paste0(nrow(counts.female)," total genes"))
y <- edgeR::DGEList(counts = counts.female, group = groups.female$Sample)
y <- edgeR::calcNormFactors(y)
keep <- rowSums(cpm(y) >= cpm.cutoff) >= min(table(groups.female$Sample)) ## is the cpm > than cutoff in two or more replicates
keep.df <- as.data.frame(table(keep))
print(paste0(keep.df[keep.df[,1] == F,2]," genes excluded using edgeR filter"))
y <- y[keep, , keep.lib.sizes = F] ## Selects genes in y that are in keep
design <- model.matrix(~groups.female$Sample)
y <- estimateDisp(y , design)
## Next two steps use Genewise negative binomial generalised linear models with quasilikelihood tests
fit <- edgeR::glmQLFit(y, design) 
qlf <- edgeR::glmQLFTest(fit, coef = 2:ncol(fit)) 
...
tpm.female.keep <- tpm.female[rownames(tpm.female) %in% rownames(y$counts),]



## Generate Dendrogram
tpm_dendrogram <- pvclust::pvclust(as.data.frame(t(scale(t(log2(tpm.female.keep+1))))), method.dist="cor", method.hclust="average",
                                   nboot=1000, parallel=TRUE)

pdf(paste0(FIGURE_DIR, "/dendrogram_females_only.pdf"),
    width = 10,
    height = 10)
  plot(tpm_dendrogram)
invisible(dev.off())
...
sample_order <- factor(as.character(dendro_order$label), levels = as.character(dendro_order$label))
groups.female <- factor_groups(groups.female[order(match(groups.female$Replicate, as.character(sample_order))),])

rownames(groups.female) <- NULL

tpm.female.keep <- tpm.female.keep[,levels(groups.female$Replicate)]


## Plot genes that passed CPM filter
log2tpm.de <- log2(tpm.female.keep + 1)
zscore.log2tpm.de <- as.data.frame(t(scale(t(log2tpm.de))))
zscore.log2tpm.de <- zscore.log2tpm.de[,as.character(sample_order)]
...
colcol <- as.character(groups.female$Worm_Colour)
colcol1 <- as.character(groups.female$Treatment_Colour)
colcol2 <- as.character(groups.female$Exposure_Colour)
colcol3 <- cbind(colcol, colcol1, colcol2)
colnames(colcol3) <- c('LifeStage', 'TreatmentColour', 'ExposureColour')
...
pdf(paste0(FIGURE_DIR, "/heatmap_of_genes_in_females.pdf"),
    width = 20,
    height = 10)
heatmap.3(heatmap_matrix,
              col=hmcol,
...
## Microfilariae
```{r}
groups.mf <- factor_groups(groups %>% filter(LifeStage == "Microfilaria"))
counts.mf <- counts[,colnames(counts) %in% groups.mf$Replicate]
tpm.mf <- tpm[,colnames(tpm) %in% groups.mf$Replicate]

## Calculate Differential Expression
FDRcutoff <- 0.05
cpm.cutoff <- 5/min(colSums(counts.mf)) * 1000000 ## list minimum counts per million
print(paste0(nrow(counts.mf)," total genes"))
y <- edgeR::DGEList(counts = counts.mf, group = groups.mf$Sample)
y <- edgeR::calcNormFactors(y)
keep <- rowSums(cpm(y) >= cpm.cutoff) >= min(table(groups.mf$Sample)) ## is the cpm > than cutoff in two or more replicates
keep.df <- as.data.frame(table(keep))
print(paste0(keep.df[keep.df[,1] == F,2]," genes excluded using edgeR filter"))
y <- y[keep, , keep.lib.sizes = F] ## Selects genes in y that are in keep
design <- model.matrix(~groups.mf$Sample)
y <- estimateDisp(y , design)
## Next two steps use Genewise negative binomial generalised linear models with quasilikelihood tests
fit <- edgeR::glmQLFit(y, design) 
qlf <- edgeR::glmQLFTest(fit, coef = 2:ncol(fit)) 
...
tpm.mf.keep <- tpm.mf[rownames(tpm.mf) %in% rownames(y$counts),]



## Generate Dendrogram
tpm_dendrogram <- pvclust::pvclust(as.data.frame(t(scale(t(log2(tpm.mf.keep+1))))), method.dist="cor", method.hclust="average",
                                   nboot=1000, parallel=TRUE)

pdf(paste0(FIGURE_DIR, "/dendrogram_mf_only.pdf"),
    width = 10,
    height = 10)
  plot(tpm_dendrogram)
invisible(dev.off())
...
sample_order <- factor(as.character(dendro_order$label), levels = as.character(dendro_order$label))
groups.mf <- factor_groups(groups.mf[order(match(groups.mf$Replicate, as.character(sample_order))),])

rownames(groups.mf) <- NULL

tpm.mf.keep <- tpm.mf.keep[,levels(groups.mf$Replicate)]


## Plot genes that passed CPM filter
log2tpm.de <- log2(tpm.mf.keep + 1)
zscore.log2tpm.de <- as.data.frame(t(scale(t(log2tpm.de))))
zscore.log2tpm.de <- zscore.log2tpm.de[,as.character(sample_order)]
...
colcol <- as.character(groups.mf$Worm_Colour)
colcol1 <- as.character(groups.mf$Treatment_Colour)
colcol2 <- as.character(groups.mf$Exposure_Colour)
colcol3 <- cbind(colcol, colcol1, colcol2)
colnames(colcol3) <- c('LifeStage', 'TreatmentColour', 'ExposureColour')
...
pdf(paste0(FIGURE_DIR, "/heatmap_of_genes_in_mf.pdf"),
    width = 20,
    height = 10)
heatmap.3(heatmap_matrix,
              col=hmcol,
