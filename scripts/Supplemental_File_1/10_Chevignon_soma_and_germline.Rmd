---
title: "Reanalysis of Chevignon, G., Soma and germline tissue expression PRJEB40568"
...
colnames(groups) <- c("Replicate", "Project", "Sample")

groups <- groups %>% mutate(column_colour = ifelse(grepl("BmBodyWall", groups$Sample), "red",
                                                   ifelse(grepl("BmMeioticZone", groups$Sample), "blue",
                                                          ifelse(grepl("BmProliferativeZone", groups$Sample), "green", "NA"))))
...
print(
 ggplot(varPart.df.all, aes(x = Variance_Explained)) + geom_histogram() + facet_wrap(~attribute, ncol = 1) + 
   labs(x = 'Variance explained ', title = 'Chevignon et al 2021 Dual RNA-Seq')
)
dev.off()
...
print(
  ...
  geom_boxplot() + labs(y = 'Variance explained (%)', title = 'Chevignon et al 2021 Dual RNA-Seq')
)
dev.off()
